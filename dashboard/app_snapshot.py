"""
Customer Analytics & Churn Prediction Platform (snapshot edition)
Self-contained Streamlit dashboard that reads a small pre-computed data
snapshot (dashboard/data/*.parquet) instead of the full DuckDB warehouse, so
it runs from a fresh git clone with no pipeline step. See app.py for the
live version backed by the DuckDB warehouse, and
scripts/export_dashboard_data.py for how the snapshot is produced.

Run with:  streamlit run app_snapshot.py
"""
import os
import sys
...
from config import ANALYSIS_DATE, CHURN_RECENCY_DAYS
...
# ----------------------------------------------------------------------------
# Data access - small pre-computed snapshot (see scripts/export_dashboard_data.py)
# ----------------------------------------------------------------------------
...
cohort = load_table("mom_cohort_retention")
category_margins = load_table("category_profit_margins")
rfm = load_table("customer_rfm")
segments = load_table("customer_segments")
churn = load_table("churn_predictions")
importance = load_table("churn_feature_importance")
